...
-- empty database   
DROP DATABASE IF EXISTS stats19;
CREATE DATABASE stats19;
USE stats19;
...
CREATE TABLE Local_Authority_Highway (code char(9) PRIMARY KEY, label text );
...
(
Accident_Index char(15) PRIMARY KEY,
...
Local_Authority_Highway char(9) REFERENCES Local_Authority_Highway(code),
...
(
Accident_Index char(15) REFERENCES accident(Accident_Index),
...
-- Holds GIS location of accident locations
CREATE TABLE accident_location 
(Accident_Index char(15) REFERENCES accident(Accident_Index),
 g POINT NOT NULL,
 SPATIAL INDEX(g)
) ENGINE=MyISAM;
CREATE INDEX acc_loc_id_idx ON accident_location(Accident_Index);
