...
CREATE OR REPLACE VIEW accident_vehicle_label AS
 select v.accident_index as accident_index, GROUP_CONCAT(vt.label) as vehicle_labels
...
-- accidents with bus.
CREATE OR REPLACE VIEW bus_accident AS
  select distinct a.accident_index as accident_index
  from accident a, vehicle v, vehicle_type vt
  where a.accident_index=v.accident_index 
  and v.vehicle_type = vt.code
  and vt.label in ('Bus or coach (17 or more pass seats)');

-- accidents with death.
CREATE OR REPLACE VIEW death AS
  select distinct a.accident_index as accident_index
  from accident a, casualty c, casualty_severity cs
  where a.accident_index=c.accident_index 
  and c.casualty_severity = cs.code
  and cs.label in ('Fatal');


-- accidents for vulnerables, pedestrians and cyclists.
CREATE OR REPLACE VIEW vulnerable_accident AS
  select distinct a.accident_index as accident_index
...
CREATE OR REPLACE VIEW pedestrian_accident AS
  select distinct a.accident_index as accident_index
  from accident a, casualty c 
  where a.accident_index=c.accident_index 
...
CREATE OR REPLACE VIEW cycle_accident AS
  select distinct a.accident_index as accident_index
  from accident a, vehicle v
  where a.accident_index=v.accident_index 
  and v.vehicle_type in (1);
  
-- accidents involving injured cyclists.
CREATE OR REPLACE VIEW cycle_injury AS
  select distinct a.accident_index as accident_index
  from accident a, casualty c 
  where a.accident_index=c.accident_index 
...
CREATE OR REPLACE VIEW vehiclev AS
    ...
    left join Vehicle_Manoeuvre vm on (Vehicle_Manoeuvre=vm.code)
    left join Junction_Location jl on (Junction_Location=jl.code)
;

-- find other vehicles involved vulnerable road user accidents. Not sure how to translate to MySQL, EXCEPT is PostGres syntax
...
-- Single vehicle colisions involving a pedestrian injury
CREATE OR REPLACE VIEW single_vehicle_pedestrian_injury AS
	select a.accident_index
    from accident a, pedestrian_accident p
    WHERE a.Number_of_Vehicles = 1
     AND p.Accident_Index = a.Accident_Index
;